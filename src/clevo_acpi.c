...
static int clevo_acpi_probe(struct platform_device *pdev)
{
	...
	status = acpi_dev_install_notify_handler(device, ACPI_ALL_NOTIFY, clevo_acpi_notify, device);
	if (status) {
...
#endif
{
	...
	if (device)
		acpi_dev_remove_notify_handler(device, ACPI_ALL_NOTIFY, clevo_acpi_notify);
